...
where
    P: Prover<BN254, VerifierMessage = Option<BN254>, ProverMessage = Option<Vec<BN254>>>,
    P::ProverConfig: crate::prover::ProductProverConfig<BN254, MemoryStream<BN254>>,
{
    ...
    let s1: MemoryStream<BN254> = MemoryStream::new_from_lex::<crate::order_strategy::SignificantBitOrder>(evals.clone()).into();
    let s2 = s1.clone();
    let s3 = s1.clone();

    let claim: BN254 = multivariate_product_claim(vec![s1.clone(), s2.clone(), s3.clone()]);

    // Prove
    let _transcript: ProductSumcheck<BN254, 3> = ProductSumcheck::<BN254, 3>::prove::<MemoryStream<BN254>, P>(
        &mut Prover::<BN254>::new(<P::ProverConfig as crate::prover::ProductProverConfig<_, _>>::default(
            claim,
            num_variables,
            vec![s1, s2, s3],
        )),
...
fn d3_time_prover() {
    ...
    run_d3_single::<TimeProductProver<BN254, MemoryStream<BN254>, 3>>(6);
}

#[test]
fn d3_space_prover() {
    use crate::multilinear_product::SpaceProductProver;
    run_d3_single::<SpaceProductProver<BN254, MemoryStream<BN254>, 3>>(6);
}

#[test]
fn d3_blendy_prover() {
    use crate::multilinear_product::BlendyProductProver;
    run_d3_single::<BlendyProductProver<BN254, MemoryStream<BN254>, 3>>(6);
}

#[test]
fn d3_streaming_eval_prover() {
    use crate::multilinear_product::StreamingEvalProductProver;
    run_d3_single::<StreamingEvalProductProver<BN254, MemoryStream<BN254>, 3>>(6);
}
...
fn run_compare_lengths<const D: usize>(num_variables: usize) {
    ...
    let time_transcript: ProductSumcheck<BN254, D> = ProductSumcheck::<BN254, D>::prove::<MemoryStream<BN254>, TimeProductProver<BN254, MemoryStream<BN254>, D>>(
        &mut Prover::<BN254>::new(crate::multilinear_product::TimeProductProverConfig { claim, num_variables, streams: streams.clone() }),
        &mut ark_std::test_rng(),
    );
    ...
    let space_transcript: ProductSumcheck<BN254, D> = ProductSumcheck::<BN254, D>::prove::<MemoryStream<BN254>, SpaceProductProver<BN254, MemoryStream<BN254>, D>>(
        &mut Prover::<BN254>::new(crate::multilinear_product::SpaceProductProverConfig { claim, num_variables, streams: streams.clone() }),
        &mut ark_std::test_rng(),
    );
    ...
    let blendy_transcript: ProductSumcheck<BN254, D> = ProductSumcheck::<BN254, D>::prove::<MemoryStream<BN254>, BlendyProductProver<BN254, MemoryStream<BN254>, D>>(
        &mut Prover::<BN254>::new(crate::multilinear_product::BlendyProductProverConfig { claim, num_stages: 2, num_variables, streams: streams.clone() }),
        &mut ark_std::test_rng(),
    );

    // Streaming evaluation-based
    let streaming_transcript: ProductSumcheck<BN254, D> = ProductSumcheck::<BN254, D>::prove::<MemoryStream<BN254>, StreamingEvalProductProver<BN254, MemoryStream<BN254>, D>>(
        &mut Prover::<BN254>::new(crate::multilinear_product::StreamingEvalProductProverConfig { claim, num_stages: 2, num_variables, streams }),
        &mut ark_std::test_rng(),
    );
